...
exports.sortChunk = function(usedModules, allFileSort) {
  let modules = Array.from(usedModules);

  let cssmodule = []; // 普通css 就是按照引用顺序 先引用的放在前面 css的module放前面 只有web端会引用
  let cmlmodule = [];
  let result = [];
  modules.forEach(item=>{
    if(/(\.cml|\.vue)$/.test(item.issuer.resource)) {
      cmlmodule.push(item);
    } else {
      cssmodule.push(item);
    }
  ...
  cmlmodule.sort(function(a, b) {
    let aIndex = allFileSort.indexOf(a.issuer.resource);
    let bIndex = allFileSort.indexOf(b.issuer.resource);
