...
//解析属性上的  :  以及 @  v-on这样的语法；
_.preParseTemplate = function(source, options) {
  ...
  let htmlArr = _.preParseHTMLtoArray(source, options, callbacks);
  debugger;
  let newHtmlArr = [];
...
_.preParseHTMLtoArray = function(html, options, callbacks) {
  ...
  function parseStartTag () {
    ...
    if (start) {
      ...
      if (end) {
        ...
        let attrString = startCallback(matchStart, options) || '';
        let content ;
...
_.startCallback = function(matchStart, options) {
  let usingComponents = options.usingComponents || [];
  let buildInComponents = options.buildInComponents || {};
