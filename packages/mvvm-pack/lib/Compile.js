...
const CMLParser = require('@didi/mvvm-template-parser');
const CMSSParser = require('@didi/mvvm-style-parser');
const JSParser = require('@didi/mvvm-script-parser');
const fs = require('fs');
  ...
  async compileModule(currentNode) {
    switch (currentNode.moduleType) {
      case 'CML':
        ...
        break;
      case 'CMSS':
        await this.compileCMSS(currentNode);
        break;
      case 'JS':
        await this.compileJS(currentNode);
        break;
      case 'JSON':
    ...
    if (currentNode.ext === '.cml') {
      ...
      Object.keys(targetObject.usingComponents).forEach(key => {
        ...
        if (cmlUtils.isFile(filePath)) {
          ...
          currentNode.parent.childrens.push(subNode);
